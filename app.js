...
let state=JSON.parse(localStorage.getItem('mfcPlannerV65')||'null');
if(!state){
...
function save(){localStorage.setItem('mfcPlannerV65',JSON.stringify(state));}
...
function autoFillFreedSpace(){
  syncStorageWithProcessZones();
  // Стеллажи виртуальные, поэтому после изменения storage они
  // автоматически перестраиваются функцией rackPlan().
}
