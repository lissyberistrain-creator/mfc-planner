...
let state=JSON.parse(localStorage.getItem('mfcPlannerV66')||'null');
if(!state){
...
function save(){localStorage.setItem('mfcPlannerV66',JSON.stringify(state));}
...
function renderTabs(){
  ...
  <table class="tbl"><tr><th>Аналитика</th><th>Значение</th></tr>${tr('Доля хранения',fmt1(a.storageArea/a.area*100)+'%')}${tr('Доля процессных зон',fmt1(a.processArea/a.area*100)+'%')}${tr('Доля staff/service',fmt1(a.supportArea/a.area*100)+'%')}${tr('Секций на 1 м²',fmt1(a.rp.total/a.area))}
