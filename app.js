...
let state=JSON.parse(localStorage.getItem('mfcPlannerV68')||'null');
if(!state){
...
function save(){localStorage.setItem('mfcPlannerV68',JSON.stringify(state));}
...
function syncStorageWithProcessZones(){
  ...
  const processZones=state.zones.filter(z=>z.type==='process' && ['Приёмка','Сборка','Отгрузка'].includes(z.name));
...
function estimatedRackableArea(){
  const areaRect=rackCandidateArea();
  const blockers=rackBlockers();
  let blocked=0;

  blockers.forEach(b=>{
    if(!rectsOverlap(areaRect,b)) return;
    const x1=Math.max(areaRect.x,b.x), x2=Math.min(areaRect.x+areaRect.w,b.x+b.w);
    const y1=Math.max(areaRect.y,b.y), y2=Math.min(areaRect.y+areaRect.h,b.y+b.h);
    blocked += Math.max(0,x2-x1)*Math.max(0,y2-y1);
  });

  return Math.max(0,area(areaRect)-blocked);
}
...
function draw(){
  ...
  // Фактическая складская площадь теперь визуализируется по реальному rack-plan.
  const actualRackPlan=rackPlan();
  const candidateArea=rackCandidateArea();
  const blockers=rackBlockers();

  // Легкий фон всей площади, потенциально доступной под хранение.
  add('rect',{
  ...
  // Подсветка свободных мест, куда секция сейчас не встала.
  const freeStep=0.5;
  for(let gx=candidateArea.x; gx<candidateArea.x+candidateArea.w; gx+=freeStep){
    for(let gy=candidateArea.y; gy<candidateArea.y+candidateArea.h; gy+=freeStep){
      const cell={x:gx,y:gy,w:freeStep,h:freeStep};
      if(blockers.some(b=>rectsOverlap(cell,b))) continue;

      const occupied=(actualRackPlan.racks||[]).some(r=>rectsOverlap(cell,r));
      if(!occupied){
        add('rect',{
          x:ox+gx*sc,
          y:oy+gy*sc,
...
function findEntityAtPoint(pt){
  // Приоритет: пользовательские объекты -> колонны -> маленькие зоны -> большие зоны.
  ...
  const candidates=state.zones
    ...
    .filter(x=>pointInRect(pt,x.z))
    .sort((a,b)=>a.a-b.a);

  if(candidates.length) return {kind:'zone',index:candidates[0].index};
  return null;
}
